const data = {
    general: {
        ...
        SUDO_PERMISSION:
            "```Hey there, I am 💠XA💠. I guess you were trying to use my commands in``` *{XA.groupName}*```. However, the bot is currently in {worktype} mode. This enables only the owner and sudo users to use the command``` *{commandName}* ```.\n\nIf you are keen to use my features, you can deploy XA on your own account without much effort, in less than 5 minutes! Check out the links given below.```\n\n🔗https://myXA.com\n🔗https://github.com/XAOfficial/XA",
        ERROR_TEMPLATE:
        ...
        SUCCESSFUL_CONNECTION:
            "*XA successfuly integrated.*\n```Bot is currently working in``` *{worktype}* ```mode.```\n\nThe bot will not work in this chat.",
    },
    abl: {
    ...
    getdp: {
        ...
        EXTENDED_DESCRIPTION:
            "```Get the profile picture of the group in a group conversation or the profile picture of XA itself in personal chat. Tag a person in a group to get their profile picture.```",
        IMAGE_CAPTION: "```Here is the display image. Procured by XA.```",
    ...
    help: {
        ...
        TEMPLATE: "\n\n *- Command* : ```{}```\n *- Info* : ```{}```",
        COMMAND_INTERFACE: "🌀 *XA Command Interface* 🌀\n\n",
